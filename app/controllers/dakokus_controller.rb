class DakokusController < ApplicationController
  ...
  def create
    ...
    # 同じ日にすでに打刻がないかチェック
    if current_employee.dakokus.where(dakoku_type: dakoku_params[:dakoku_type]).present?
      ...
      is_success = ActiveRecord::Base.transaction do
        attendance = if dakoku_params[:dakoku_type] == 'syukkin'
                       current_employee.attendances.create(attendance_date: dakoku_datetime.to_date.to_s)
                     else
                       current_employee.attendances.find_by(attendance_date: dakoku_datetime.to_date.to_s)
                     end
